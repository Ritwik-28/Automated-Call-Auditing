...
# Adjusting the ask_chatgpt function to handle message pagination correctly

# Assuming these are set and loaded from your environment or configuration
api_URI = "https://growth-team-ai.openai.azure.com/"
api_KEY = os.getenv("OPENAI_API_KEY")
api_version = "2024-02-15-preview"
fixed_thread_id = "thread_UUBsfaRPVfG0TPRnNPXZmxcK"
assistant_id = "asst_XlHNObFbrbl0kCyvt4FwGwcI"

def ask_chatgpt(prompt, synchronized_text):
    ...
    # Create a new Message within the existing thread
    message_creation_response = client.beta.threads.messages.create(
        thread_id=thread_id,
        role="user",
        content=synchronized_text + "\n\n" + prompt,
    )

    # Extract the message ID of the newly created message
    last_user_message_id = message_creation_response.id

    # Create a new Run using the assistant and the fixed thread
    run = client.beta.threads.runs.create(
        thread_id=thread_id,
        assistant_id=assistant_id,
        instructions="Ensure the feedback generated is as per the Feedback Format document and also ensure that each parameter has a detailed feedback containing rating, areas of improvements and suggestions to improve. Also include Duration analysis for the entire conversation.",
    )
        ...
        if run_status.status == "completed":
            ...
            for message in messages:
                ...
                if message.id == last_user_message_id:
                    found_last_user_message = True  # Next assistant message is the response
...
# transcript = "User: What's the weather like? Assistant: It's sunny and 75 degrees."
# ask_chatgpt(prompt, transcript)
    ...
    for i, row_main in enumerate(rows_main, start=2):
        try:
            ...
            try:
                ...
                openai_response = ask_chatgpt(prompt, synchronized_text)
